...
public class GestionCSV {
    ...
    public static List<Empleado> leerCsvYcrearObjeto2(String ruta) {
    // Fichero a leer con datos de ejemplo
    String idFichero = ruta;
    ...
    try (Scanner datosFichero = new Scanner(new File(idFichero), "ISO-8859-1")) {
        ...
        while (datosFichero.hasNextLine()) {
            ...
            auxE.setApellido(tokens[0].replaceAll("\"\"", ""));
            auxE.setNombre(tokens[1].replaceAll("\"\"", ""));
            auxE.setDni(tokens[2].replaceAll("\"\"", " "));
            auxE.setPuesto(tokens[3].replaceAll("\"\"", ""));
            if (tokens[4].equals(" ")) {
                auxE.setFechaPosesion(null);
            } else {
                 String fechaString = tokens[4].trim();
            ...
            if (tokens[5].equals(" ")) {
                ...
                String fechaString = tokens[5].trim();
                DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");
                LocalDate fecha = LocalDate.parse(fechaString, formatter);
                auxE.setFechaCese(fecha);
            }
            auxE.setTelefono(tokens[6].replaceAll("\"\"", ""));
            auxE.setEvaluador(Boolean.parseBoolean(tokens[7].replaceAll("\"\"", "")));
            auxE.setCoodinador(Boolean.parseBoolean(tokens[8].replaceAll("\"\"", "")));
